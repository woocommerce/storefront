...
This document outlines the process of releasing new versions of the Storefront theme.
...
_Outcome_: A storefront.zip file that you can test with. 
...
_Outcome_: A storefront.zip file that is available to the community and other stakeholders so they can test and provide feedback. 
...
_Outcome_: You (release lead) are confident that the product is ready for release, and a safe & worthwhile upgrade for merchants.
...
_Outcome_: Merchants are using the new Storefront, your new features are live, stores are working better, customers are happier!
