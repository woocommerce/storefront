...
describe( 'Store owner can login and make sure WooCommerce is activated', () => {
	...
	it( 'can make sure Storefront is activated. If not, activate it', async () => {
		const slug = 'storefront';
		await StoreOwnerFlow.openTheme();
		const disableLink = await page.$( `div[data-slug="${ slug }"] a.customize` );
		if ( disableLink ) {
			return;
		}
		await page.click( 'a.button.activate' );
		await page.waitForSelector( 'a.button.activate' );
...
// 		await StoreOwnerFlow.openTaskList();
